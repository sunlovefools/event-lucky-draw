-- Keep only the schema used by the current station-stamp experience.
--
-- Retired flows:
--   * form-based final surveys (the Final Survey is now a station stamp)
--   * vendor usernames and device sessions (stations use their direct links)

-- This function used final_survey_responses, so it must be replaced before
-- that legacy table is removed.
drop function if exists public.admin_participant_progress();

drop table if exists public.final_survey_responses;
drop table if exists public.vendor_sessions;
